...
extern Agg IMakeAgg ARGS((Cost cost, Count plan_size, Count plan_width, int fragment, int parallel, AggState agg state, List qptargetlist, List qpqual, PlanPtr lefttree, PlanPtr righttree, ObjectId tempid, Count keycount, Name aggname, int aggstate));
...
void set_state ARGS((Plan node , EStatePtr value ));
EStatePtr get_state ARGS((Plan node ));
...
void set_lefttree ARGS((Plan node , PlanPtr value ));
PlanPtr get_lefttree ARGS((Plan node ));
void set_righttree ARGS((Plan node , PlanPtr value ));
PlanPtr get_righttree ARGS((Plan node ));
...
void set_frag_parent_frag ARGS((Fragment node , FragmentPtr value ));
FragmentPtr get_frag_parent_frag ARGS((Fragment node ));
...
void set_ruleinfo ARGS((Join node , JoinRuleInfoPtr value ));
JoinRuleInfoPtr get_ruleinfo ARGS((Join node ));
...
Plan MakePlan ARGS((Cost cost , Count plan_size , Count plan_width , int fragment , int parallel , EStatePtr state , List qptargetlist , List qpqual , PlanPtr lefttree , PlanPtr righttree ));
...
Plan IMakePlan ARGS((Cost cost , Count plan_size , Count plan_width , int fragment , int parallel , EStatePtr state , List qptargetlist , List qpqual , PlanPtr lefttree , PlanPtr righttree ));
Plan RMakePlan ARGS((void ));
void RInitFragment ARGS((Pointer p ));
Fragment MakeFragment ARGS((Plan frag_root , Plan frag_parent_op , int frag_parallel , List frag_subtrees , FragmentPtr frag_parent_frag , List frag_parsetree , bool frag_is_inprocess ));
...
Fragment IMakeFragment ARGS((Plan frag_root , Plan frag_parent_op , int frag_parallel , List frag_subtrees , FragmentPtr frag_parent_frag , List frag_parsetree , bool frag_is_inprocess ));
...
Join MakeJoin ARGS((JoinRuleInfoPtr ruleinfo ));
...
Join IMakeJoin ARGS((JoinRuleInfoPtr ruleinfo ));
