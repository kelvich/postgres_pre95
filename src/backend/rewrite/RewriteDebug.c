...
void
AddQualifications ( parsetree , new_qual )
     List parsetree;
     List new_qual;
{
    if ( parse_qualification(parsetree) == NULL )
      parse_qualification(parsetree) = new_qual;
    else
     ...
     List user_parsetree;
{
	...
	foreach ( j , ruletrees ) {
	    ...
	    AddQualifications ( user_parsetree , rule_qual );
...
List 
FindMatchingNew ( user_parsetree , attno )
     List user_parsetree;
     int attno;
{
    List tlist = parse_targetlist(user_parsetree);
    List i = NULL;
     ...
     int offset;
{
	...
	if ( thisnode && IsA(thisnode,Var)) {
		 ...
		 * and stick it here
		 */
		matched_new = FindMatchingNew ( user_parsetree ,
...
ModifyUpdateTlist( tlist , user_parsetree , current_varno )
     List tlist;
     List user_parsetree;
     int current_varno;
{
    List i 		= NULL;
    int new_trigger_varno = current_varno + length(tlist) - 2;

    /* replace #S(Var :varno2 ...) with corresponding RHS of
     * tlist expression from user_parsetree
     */
    foreach ( i , tlist ) {
	List this_entry = CAR(i);
	Resdom this_resdom = (Resdom)CAR(this_entry);
	List original_rhs = CDR(this_entry);
	List new_rhs = NULL;

	new_rhs = HandleVarNodes ( original_rhs , user_parsetree ,
				   new_trigger_varno, -2 );
	CDR(this_entry) = new_rhs;
    }
}
/*
 * ModifyUpdateNodes
