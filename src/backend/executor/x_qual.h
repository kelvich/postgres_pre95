/* $Header$ */
extern int sysattrlen ARGS((AttributeNumber attno));
extern bool ExecQualClause ARGS((List clause, ExprContext econtext));
extern bool ExecQual ARGS((List qual, ExprContext econtext));
extern List ExecTargetList ARGS((List targetlist, int nodomains, AttributePtr targettype, Pointer values, ExprContext econtext));
extern Const ExecMakeVarConst ARGS((List tuple, AttributeNumber attnum, AttributePtr tuple_type, Buffer buffer, ExprContext econtext));
...
extern Const ExecEvalArrayRef ARGS((Expr expression, ExprContext econtext));
